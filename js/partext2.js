const canvas = document.querySelector("#comming-soon");
...
function getTextCoordinates(text) {
	...
	const fontSize = Math.min(canvas.width / 2, canvas.height / 2);
	ctx.font = `900 ${fontSize}px Arial`;
	ctx.fillStyle = "white";
	ctx.textAlign = "center";
	// ctx.textBaseline = "middle";
	ctx.fillText(text, canvas.width / 2, canvas.height / 2);
	// ctx.fillText(text, canvas.width / 1, canvas.height / 1);
